...
public class FileListAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        holder.fileItemView.setAdapter(this);
    ...
    public void selectAll() {
        ...
        for (Iterator<FileItem> iterator = list.iterator(); iterator.hasNext();) {
            ...
            if (mode == FileDialog.FILE_MODE_OPEN_FILE_MULTI && fileItem.isDirectory()) {
                // fileItem是目录，但是只能选择文件，则返回
                break;
            }
            if (mode == FileDialog.FILE_MODE_OPEN_FOLDER_MULTI && !fileItem.isDirectory()) {
    ...
    public void selectOne(FileItem fileItem) {
        ...
        if (mode > FileDialog.FILE_MODE_OPEN_FILE_MULTI) {
            // 如果是单选
            if (mode == FileDialog.FILE_MODE_OPEN_FILE_SINGLE && fileItem.isDirectory()) {
                // fileItem是目录，但是只能选择文件，则返回
                return;
            }
            if (mode == FileDialog.FILE_MODE_OPEN_FOLDER_SINGLE && !fileItem.isDirectory()) {
                // fileItem是文件，但是只能选择目录，则返回
                return;
            }
            for (Iterator<FileItem> iterator = list.iterator(); iterator.hasNext();) {
